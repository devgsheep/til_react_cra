...
- HTML 태그 형태를 리턴하는 함수
- 화살표 함수(`rafce`)이든, 일반함수(`rfce`)이든 `HTML 태그를 리턴`하면 인정
- JSX는 `반드시 하나의 태그`에 `포함되도록 배치`하여야 한다.
- `<></>`를 `Fragment` 라고 합니다.
- 파일명 및 컴포넌트 함수는 반드시 `파스칼`이어야 한다.

## 2. Component와 Page 폴더 구분

- `폴더명은 반드시 소문자`로 하여야 한다
- 한 장의 html을 구성한다면 `pages 폴더`에 저장
- `하나의 html을 완성하기 위한 배치 요소`라면 `components 폴더`에 저장
...
- index.css에 모든 페이지에 적용되는 기본 파일이므로
...
- jsx : `<div className="bot">로고</div>`
- css : `.bot {background-color: red;}`
- StartPage.jsx
...
- 태그처럼 생겼다. (내가만든 태그 : 시멘틱 처럼 이해하자.)
- 태그가 내용을 설명하도록 구성한다.
- CSS 같이 배치가능 합니다. (VSCode 플러그인: `vscode-styled-components`)
- props도 전달해서 다양한 결과를 보여줄 수 있습니다.
- props는 객체라서 `.`으로 접근 가능
- props는 객체라서 `객체 구조 분해 할당` 해서 사용할 수 있다.
- 기능 확장 즉 Extends 가능합니다.
- export를 통해서 외부로 내보낼 수 있습니다.
- import해서 사용할 수 있습니다.
...
- 3차 최종버전 (components/ui/Button.jsx)
...
function Skeleton({ width = "0%", height = "0px" }) {
  // js 자리
  // keyframes css 애니메이션
  const mov = keyframes`
  ...
  // jsx 자리
  return <SkeletionBox width={width} height={height} />;
}
...
- `JS 자리`에 if문과 switch 문을 사용할 수 있음.

- `JSX 자리`에 if문과 switch 문을 사용할 수 없음.
- if문과 switch문 대신에 `3항 연산자`가 가장 많이 사용됨.
- `JSX 자리`에 falshy한 값은 출력이 안됩니다.
...
- `객체?.속성명`
- 객체가 null 이거나 undefined이면 리액트 오류
...
function Test() {
  ...
  // jsx 자리
  return (
    <Hi isLogin={true} msg={"새로운메세지"} user={{ name: "홍길동" }}>
      Test
    </Hi>
  );
}
