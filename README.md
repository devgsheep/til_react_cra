...
# useCallback (변수가 아닌 함수를 저장)

- 왜 함수를 렌더링마다 자꾸 새로 만들죠?
...
- 성능 이슈 발생 가능함.
  ...
  - 원하는 것
    - `const add = () {...}` 한번 만들고 나서 다시는 새로 만들지 마라
...
- 리액트 변수, 즉 `useState에 의해서 값이 변해서 리랜더링 됨`은 정상임.
- 그러나 리랜더링에서 제외되어야 하는 컴포넌트를 설정할 필요성이 있다.
