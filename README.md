...
const TextArea = styled.textarea`
  ...
  border-radius: 8px;
  width: 100%;
  resize: vertical; // 세로로만 커지고 작아짐
`;

const Button = styled.button`
  padding: 5px;
  background-color: #1f993a;
  color: #fff;
  border: none;
  border-radius: 8px;
  font-size: 11px;
...
- `const [isEdit, setIsEdit] = useSatate(false)`
...
- 삭제 이후 추가하면 id 중복 발생
- uuid 라이브러리 또는 일정하게 증가하는 리액트 변수 출력
...
- 웹브라우저에 보관(임시 보관)
- 데이터베이스 컴퓨터에 보관(MySQL, PostgreSQL, NonQL)

### 6.1. 웹브라우저에 보관하기(F12)

- cookie : 짧은 문자열로서 일정시간 동안 유지 후 파기
- session : 웹 브라우저에서 활용하는 동안 유지(웹 브라우저 종료시 파기)
